...
def parse_args():
    ...
    parser.add_argument('--input_disease_miRNA', nargs='?', default='..\..\data\Test\disease_miRNA.csv',
                        help='Input disease_gene_relationship file')
    parser.add_argument('--input_label',nargs = '?',default='..\..\data\Test\label.csv',
                        help='sample label')
...
def deepnn(x, keep_prob, args):
    ...
    with tf.name_scope('conv_pool'):
        ...
        regula = tf.keras.regularizers.l2(args.L2_norm)
...
def main(args):
    with tf.device('/cpu:0'):
        ...
        with tf.Session() as sess:
            ...
            saver.save(sess=sess, save_path='./model/GCSENet.ckpt')
